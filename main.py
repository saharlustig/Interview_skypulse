...
def display_frames(original_frame, aligned_frame, output_video=None):
    ...
    cv2.waitKey(1)
    if output_video is not None: output_video.write(np.concatenate((original_frame, aligned_frame), axis=0))


def main():
    config = yaml.safe_load(open(ROOT_DIR / "config.yml"))
    frame_loader = FrameLoader(ROOT_DIR / config['video_dir_relative'] / config['video_name'])

    output_video = cv2.VideoWriter(ROOT_DIR / config['video_dir_relative'] / config['output_video_name'],
                                   cv2.VideoWriter_fourcc(*"mp4v"), frame_loader.fps,
                                   (frame_loader.width, frame_loader.height * 2))

    frame_stabilizer = FrameStabilizer(frame_loader.width, frame_loader.height, buffer_size=int(frame_loader.fps * 2))
    while True:
        ret, frame = frame_loader.get_frame()
        if not ret: break
        stabilized_frame = frame_stabilizer.stabilize(frame)
        # display_frames(frame, stabilized_frame, output_video=output_video)
